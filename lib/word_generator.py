...
class WordGenerator():
    ...
    def __init__(self, stream, allow_unicode_text=False, ignore_emojis=True, remove_variation_selectors=True, break_replacement=True):
        self.stream = stream
    ...
    def get_words(self, sentence):
        ...
        # commented (and after that uncommented) - that comment solves the double generating thing (once in 02 and once in 03) (CUSTOM_AT -> custom_at)
        # + that is he parser, we dont suuport EN syntax, doents matter if it is lower or not.
        sentence = sentence.strip().lower()  # here should be done the HE parsing?
    ...
    def convert_unicode_word(self, word):
        ...
        # word = self.convert_unicode_punctuation(word)  # turns ain (the hebrew char) into `

        if self.ignore_emojis:
            emojis, word = separate_emojis_and_text(word)
...
class TweetWordGenerator(WordGenerator):
    ...
    def validated_tweet(self, data):
        ...
        # that is commented because in that case all tweets are 1 value length
    ...
    def data_preprocess_filtering(self, line, iter_i):
        # fields = line.strip().split("\t")  # tweets always 1 value length
