...
def extract_emojis(text, wanted_emojis):
    """
    now supporting multiple emojis extraction
    :param text:
    :param wanted_emojis:
    :return:
    """
    text = remove_variation_selectors(text)
    text = text.strip().split()  # text = [w1, ..., w_n] (where w_i can be e1e2e3 OR worde1)
...
def process_word(word):
    ...
    """
    word = shorten_word(word)  # "jjjj" -> "jj"
