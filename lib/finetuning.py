...
def load_benchmark(path, vocab, extend_with=0, vocab_uint=16):
    ...
    st = SentenceTokenizer(vocab, maxlen, uint=vocab_uint, ignore_sentences_with_URL_mask=False)
...
def tune_trainable(model, nb_classes, train, val, test, epoch_size,
    ...
    if batch_generator:
        ...
        train_gen = sampling_generator(X_train, y_train,
                                       batch_size, upsample=False, epoch_size=1)
