...
import numbers
import numpy as np
from lib.create_vocab import extend_vocab, VocabBuilder
# from word_generator import WordGenerator
from word_generator import TweetWordGenerator, WordGenerator
...
class SentenceTokenizer():
    ...
    def __init__(self, vocabulary, fixed_length, custom_wordgen=None,
        ...
        # originally was masking_value=0 in the function call since in the vocab CUSTOM_MASK was always=0,
        # I set it to self.vocabulary["CUSTOM_MASK"] value, since now the CUSTOM_MASK value is dynamic
        if masking_value is None:
            self.masking_value = self.vocabulary["CUSTOM_MASK"]
        else:
    ...
    def tokenize_sentences(self, sentences, reset_stats=True, max_sentences=None):
        ...
        for s_words, s_info, orig_line in self.wordgen:
