...
[[ ! -z "$DATA" ]] && echo "DATA: $DATA" || echo "[WARNING] DATA arg empty"


echo Start making vocab file
python src/make_vocab.py $TWEETS_TEXT_FILE $VOCAB_FILE $THRESHOLD $DATA &> $LOGS_DIR/vocab.log
echo Done making vocab file

echo Start making dataset pkl file
python src/make_dataset.py $TWEETS_TEXT_FILE $DATA_PKL_FILE $DATA &> $LOGS_DIR/dataset.log
